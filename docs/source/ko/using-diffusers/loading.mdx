...
[`DiffusionPipeline`] 클래스를 통해 [허깅페이스 허브에 올라온 디퓨전 모델들](https://huggingface.co/models?library=diffusers)을 불러올 수 있습니다. 다음 예제 코드를 통해 CompVis의 레이턴트 디퓨전 모델을 다운로드하는 방법을 알아보겠습니다.
...
[`DiffusionPipeline`]은 모델에 적합한 파이프라인 클래스를 자동으로 탐지하고 (이 예시의 경우, [`LDMTextToImagePipeline`]), 필요한 구성요소(config)와 가중치(weight) 파일들을 다운로드하고 캐싱합니다. 그런 다음 최종적으로 파이프라인 인스턴스를 반환합니다. 이렇게 반환된 파이프라인 인스턴스의 [`LDMTextToImagePipeline.__call__`] 메서드를 통해 이미지를 생성할 수 있습니다. (예시, `ldm("image of a astronaut riding a horse")` )
...
[`LDMTextToImagePipeline`]을 비롯한 대부분의 디퓨전 파이프라인들은 여러 개의 다양한 컴포넌트들(모델, 토크나이저, 스케줄러 등)로 구성됩니다. [파이프라인 클래스](./api/overview#diffusers-summary)는 이러한 디퓨전 시스템 내의 다양한 컴포넌트들을 통합하고, 이를 통해 보다 사용이 용이한 형태의 API를 제공함과 동시에 커스터마이징의 유연성을 보장하는 것을 목표로 합니다.

*[`LDMTextToImagePipeline`]과 [ `StableDiffusionPipeline`]의 구체적인 추론 과정에 대해서는 [여기](https://huggingface.co/blog/stable_diffusion#how-does-stable-diffusion-work)에서 확인할 수 있습니다.*



### 액세스 요청이 필요한 파이프라인

디퓨전 모델은 매우 사실적인 이미지를 생성할 수 있습니다. 그리고 이러한 특성이 선정적이거나 폭력적인 이미지를 생성하는 것에 악용될 수 있습니다. 이를 막기 위해 일부 매우 우수한 성능의 디퓨전 모델들의 경우, 해당 모델을 사용하기 전, 라이센스에 동의하도록하고 있습니다. 만약 유저가 해당 라이센스에 동의하지 않는다면, 해당 파이프라인을 다운로드 받을 수 없습니다.
...
앞서 살펴본 방법과 동일한 방법으로  [`runwayml/stable-diffusion-v1-5`](https://huggingface.co/runwayml/stable-diffusion-v1-5) 모델의 파이프라인을 로드해보겠습니다. 해당 파이프라인은 [모델 카드](https://huggingface.co/runwayml/stable-diffusion-v1-5)에 있는 라이센스에 동의하고 Hugging Face Hub에 로그인한 경우에만 작동합니다. 그렇지 않으면 아래와 같은 오류 메시지가 표시됩니다.
...
따라서 해당 파이프라인을 사용하고 싶다면 반드시 라이센스에 동의해야 합니다. 동의하는 방법은 간단합니다. 먼저 해당 [모델 카드](https://huggingface.co/runwayml/stable-diffusion-v1-5)에 접속해서 "Agree and access repository"를 클릭합니다.
...
그 다음 허깅페이스 CLI와 액세스 토큰을 사용하여 로그인하거나,

```
huggingface-cli login
```

[`DiffusionPipeline.from_pretrained`]에 `use_auth_token` 인자를 통해 [액세스 토큰](https://huggingface.co/docs/hub/security-tokens#user-access-tokens)을 전달합니다. `use_auth_token` 인자를 통해 액세스 토큰을 전달하는 경우, 따로 `huggingface-cli login`을 실행하지 않아도 됩니다.
...
이밖에도 액세스 요청이 필요한 파이프라인을 허깅페이스 허브에 접속하지 않고 사용할 수 있는 방법이 있습니다. 바로 다음 챕터에서 다뤄볼 파이프라인을 로컬로 불러오는 것입니다.
...
만약 `repo_id`가 로컬 패스라면, [`DiffusionPipeline.from_pretrained`]는 이를 자동으로 감지하여 허브에서 파일을 다운로드하지 않습니다. 일반적으로는 해당 모델의 최신 상태를 보장하기 위해 허브에서 직접 가중치(weight)를 불러오는 것이 권장됩니다. 하지만 익명성을 원하거나 독자적인 어플리케이션을 구축해야 한다면, 로컬로 파이프라인을 불러오는 방식이 필요할 것입니다.
