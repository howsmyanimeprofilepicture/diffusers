...
- 데이터셋을 Hub에 업로드하고 데이터셋 레포지토리 id를 `--dataset_name` 인수에 전달합니다.
...
💡 데이터셋을 만들고 Hub에 업로드하는 것에 대한 자세한 내용은 [🤗 데이터셋을 사용한 이미지 검색](https://huggingface.co/blog/image-search-datasets) 게시물을 참고하세요.

</Tip>

[이미지 폴더](https://huggingface.co/docs/datasets/image_load#imagefolder) 기능을 사용하여 , which creates an `image` column containing the PIL-encoded images.
